...
fn trim_rating(mut vec: Vec<String>, size: usize, mut counts: Vec<usize>, co2: bool) -> u32 {
	let mut i = size - 1;
	let mut len = vec.len();
	while vec.len() > 1 {
...
pub fn day_03_part2(file: &str) -> u32 {
	...
	let o2 = trim_rating(vec.clone(), size, counts.clone(), false);
	let co2 = trim_rating(vec, size, counts, true);
